...
app.get('/api/auth/facebook', (req, res) => {
  ...
  const redirectUri = isDev ? `${req.protocol}://${req.get('host')}/auth/facebook/callback` : process.env.FACEBOOK_REDIRECT_URI_PROD;
...
app.get('/api/auth/google', (req, res) => {
  ...
  const host = req.get('host') || '';
  const isDev = host.includes('repl.co') || host.includes('replit.dev') || host.includes('localhost');
  const redirectUri = isDev ? `${req.protocol}://${req.get('host')}/auth/google/callback` : process.env.GOOGLE_REDIRECT_URI_PROD;
  console.log('Host detected:', host, 'isDev:', isDev);
